...
app.post('/api/keys/test', (req, res) => {
    ...
    const privateKey = decrypt(secureKeyStorage.encryptedPrivateKey, process.env.KEY_ENCRYPTION_SECRET);
    
    // 测试加密/解密
    const testVote = 1; // 模拟给候选人1投1票
    const encrypted = crypto.encryptVote([testVote], secureKeyStorage.publicKey);
    const decrypted = crypto.decrypt(encrypted[0].c1, encrypted[0].c2, privateKey);
